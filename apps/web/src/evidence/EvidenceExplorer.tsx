...
export function EvidenceExplorer() {
  ...
  const hasCoverage = release?.dataset_id === 'istat_m2';
  const chosen = coverage.find((item) => `${item.period}|${item.series_code}` === selection);
  const parentLevel = hasCoverage
    ? level === 'municipality'
      ? 'province'
      : level === 'province'
        ? 'region'
  ...
  useEffect(() => {
    if (!release || (hasCoverage && !chosen)) return;
    const controller = new AbortController();
    setPage(null);
    setLoading(true);
    setError('');
    ...
    if (hasCoverage) query.set('level', level);
    ...
    return () => controller.abort();
  }, [release, after, retry, chosen, hasCoverage, level, sort, filters]);
  ...
  return (
    <>
      ...
      <main id="content">
        ...
        {hasCoverage && (
          <section className="controls coverage-controls" aria-label="Copertura demografica">
            <div>
          ...
          >
            <div className="panel-heading">
              <h2 id="observations-title">
                {hasCoverage ? 'Evidenze per territorio' : 'Popolazione per territorio'}
              </h2>
                        ...
                        <tr key={item.territory_id}>
                          <td>
                            <span className="territory-name-cell">
                              ...
                              {hasCoverage && item.level !== 'country' ? (
                                <button
                                  type="button"
        ...
        {hasCoverage && release && <TerritorialHistory key={release.id} releaseId={release.id} />}
