...
def test_readiness_requires_each_coverage_view(settings: Settings) -> None:
    with (
...
def test_repeated_upgrade_preserves_published_coverage(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, coverage_bundle: tuple[Bundle, Path]
) -> None:
...
def test_publication_api_history_boundaries_and_immutability(
    settings: Settings, coverage_bundle: tuple[Bundle, Path]
) -> None:
    bundle, contract = coverage_bundle
...
def test_sort_cursor_preserves_numeric_ties_and_nulls(settings: Settings) -> None:
    # A temporary, invented fixture exercises nulls that the complete coverage excludes.
