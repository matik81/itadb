"""Reviewed ISTAT territorial aggregates adapter using pinned original inputs."""
...
def build_territorial_aggregates(root: Path, inputs: Path, contract: Path) -> Bundle:
...
def _build(root: Path, inputs: Path, contract: Path) -> Bundle:
    ...
    if spec["name"] != "istat-m2" or spec["version"] != "1.0.0":
        raise ValueError("Unsupported territorial aggregates contract")
    ...
    print(
        f"Copertura verificata offline: {len(bundle.observations):,} osservazioni; "
