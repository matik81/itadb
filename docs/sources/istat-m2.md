# M2 — copertura territoriale e demografica ISTAT

Perimetro revisionato il 23 settembre 2026. Il contratto
[`istat-m2-v1.json`](../../contracts/istat-m2-v1.json) fissa 14 originali con URL,
...
```sh
uv run python scripts/run_logged.py --label "Acquisizione M2" -- uv run itadb fetch-m2
# Il comando stampa il percorso del nuovo inventario; riusarlo nei due passi seguenti.
uv run itadb check-m2 --inputs data/state/m2-inputs-UUID.json
uv run python scripts/run_logged.py --label "Pubblicazione M2" -- uv run itadb ingest-m2 --inputs data/state/m2-inputs-UUID.json
```

`fetch-m2` riusa gli originali il cui hash è verificato, altrimenti acquisisce
...
M2 non produce popolazione sintetica. Il pilota è M3; la scala nazionale 1:1,
circa 60 milioni di individui virtuali, appartiene a M4 e richiede misure proprie.
